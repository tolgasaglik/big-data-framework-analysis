...
FLINK_HOME=$HOME/big-data-framework-analysis/incubator-flink
APP=$HOME/big-data-framework-analysis/incubator-flink/flink-dist/target/flink-dist_2.11-1.8-SNAPSHOT.jar
OPTS=100G
#################################################
...
export SPARK_TOTAL_EXECUTER_CORES=$((${SLURM_NNODES}*${SLURM_NTASKS}*(${SPARK_WORKER_CORES} -1) ))
...
if [ -n "${MODE_INTERACTIVE}" ]; then
    ...
    spark-submit \
      ...
      ${FLINK_HOME}/target/spark-terasort-1.1-SNAPSHOT-jar-with-dependencies.jar 5g \
...
time spark-submit \
      ...
      org.apache.flink.test.recordJobs.sort.TeraSort
      ${APP} \
